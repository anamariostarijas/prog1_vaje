...
(* Nat_int.eq NAt_int.zero Nat_int.zero;;
let open Nat_int in eq zero zero
*)
...
(* Write another implementation of NAT, taking inspiration from the Peano
 ...
 - The carrier type is given by a variant type that says that a natural number
   is either zero or the successor(naslednjik) of another natural number.
...
module Nat_Peano : NAT = struct
  ...
  let rec ad x y =
	match (x,y) with
	|(Zero, Zero) -> Zero
	|(Successor x, Zero) -> Successor x
	|(Successor x, Successor y) -> Successor (ad x y)
	
  let rec sub x y =
	match (x,y) with
	|(Zero, y) -> y
	|(Successor x, Zero) -> Successor x
	|(Successor x, Successor y) -> Successor (sub x y)
