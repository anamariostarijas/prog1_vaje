...
let rec podvoji l acc= 
	match  l with
	| [] -> acc 
	| x::xs -> podvoji xs (acc@[x;x])
...
let rec razdeli k l =
	...
	|(_, [])-> ([], [])
	|(0, l) -> ([], l)
	|(k, x::xs) -> let (l1, l2) = razdeli (k-1) xs in
				(x::l1, l2)
...
(* Funkcija "max_po_komponentah l1 l2" vrne seznam, ki ima za elemente
 ...
 - : int list = [5; 4; 3; 3; 4]
 ---------- *)
let rec max_po_komponentah l1 l2 acc = 
	match (l1, l2) with
	|(_, [])|([],_)-> acc
	|(x::xs, hd::tl) -> if x>hd then max_po_komponentah xs tl (acc@[x]) else
						max_po_komponentah xs tl (acc@[hd]) 
